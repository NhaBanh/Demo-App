...
actor MockInventoryServer {
    private var buyItems: [RemoteBuyItemDTO]
    private var sellItemsByID: [UUID: RemoteSellItemDTO]

    init(
        buyItems: [RemoteBuyItemDTO] = MockInventoryServer.defaultBuyItems(),
        sellItems: [RemoteSellItemDTO] = []
    ) {
        self.buyItems = buyItems
        self.sellItemsByID = Dictionary(uniqueKeysWithValues: sellItems.map { ($0.id, $0) })
    }

    func fetchBuyItems(query: ToBuyCatalogQuery) async throws -> [RemoteBuyItemDTO] {
        try await Task.sleep(for: .milliseconds(180))

        let searchText = query.searchText.trimmingCharacters(in: .whitespacesAndNewlines)
        var filtered = searchText.isEmpty ? buyItems : buyItems.filter {
    ...
    func fetchSellItems() async throws -> [RemoteSellItemDTO] {
        ...
        return sellItemsByID.values.sorted { $0.updatedAt > $1.updatedAt }
    }

    func createSellItem(_ item: RemoteSellItemDTO) async throws {
        try await Task.sleep(for: .milliseconds(120))
        sellItemsByID[item.id] = item
    }
    ...
    func deleteSellItem(id: UUID) async throws {
        ...
        sellItemsByID.removeValue(forKey: id)
    }

    func markSellItemSold(id: UUID) async throws {
        try await Task.sleep(for: .milliseconds(100))
        guard let existing = sellItemsByID[id] else { return }

        sellItemsByID[id] = RemoteSellItemDTO(
            id: existing.id,
            name: existing.name,
...
private extension MockInventoryServer {
    static func defaultBuyItems() -> [RemoteBuyItemDTO] {
        [
